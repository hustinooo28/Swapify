...
import {
  ...
  Modal, TextInput, Alert, ActivityIndicator, Linking, Platform,
} from 'react-native';
...
export default function ItemDetailScreen() {
  ...
  // Live item status — re-fetch so we always show the latest
  ...
  const fetchLatestItemStatus = async () => {
    const { data } = await supabase
      .from('items')
      .select('status')
      .eq('id', item.id)
      .single();
    if (data) setItemStatus(data.status);
  };
  ...
  const openInMaps = () => {
    if (!mapCoords) return;
    Linking.openURL(`https://maps.google.com/?q=${mapCoords.lat},${mapCoords.lng}`);
  };

  const isOwnItem = currentUser?.id === item.user_id;
  const isTraded = itemStatus === 'traded';
  const isPending = itemStatus === 'pending';
  const isUnavailable = isTraded || isPending;
  const conditionColor = getConditionColor(item.condition || 'Good');

  const unavailableLabel = isTraded ? 'This item has been traded' : 'This item is pending a trade';
  const unavailableIcon = isTraded ? 'checkmark-done-circle' : 'time';
  const unavailableColor = isTraded ? Colors.success : Colors.warning;
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.imageContainer}>
          <Image
            ...
            style={[styles.image, isUnavailable && styles.imageGrayscale]}
            resizeMode="cover"
          />
          ...
          <View style={[styles.categoryOverlay, { backgroundColor: theme.surface }]}>
            <Text style={[styles.categoryOverlayText, { color: theme.textSecondary }]}>{item.category}</Text>
          </View>

          {/* Unavailable overlay banner on image */}
          {isUnavailable && (
            <View style={[styles.soldBanner, { backgroundColor: unavailableColor }]}>
              <Ionicons name={unavailableIcon as any} size={14} color="#fff" />
              <Text style={styles.soldBannerText}>{isTraded ? 'TRADED' : 'PENDING'}</Text>
            </View>
          )}
        </View>
        ...
        <View style={[styles.contentCard, { backgroundColor: theme.surface }]}>
          ...
          <View style={styles.topRow}>
            <View style={[styles.priceBadge, isUnavailable && { backgroundColor: theme.background }]}>
              <Text style={[styles.priceText, isUnavailable && { color: theme.textLight }]}>
          ...
          <Text style={[styles.title, { color: isUnavailable ? theme.textLight : theme.text }]}>
            {item.title}
          </Text>

          {/* Unavailable notice */}
          {isUnavailable && (
            <View style={[styles.unavailableNotice, { backgroundColor: unavailableColor + '18' }]}>
              <Ionicons name={unavailableIcon as any} size={16} color={unavailableColor} />
              <Text style={[styles.unavailableText, { color: unavailableColor }]}>
                {unavailableLabel}
              </Text>
            </View>
          )}
          ...
          >
            {item.user?.avatar_url ? (
              <Image source={{ uri: item.user.avatar_url }} style={styles.sellerAvatarImg} />
            ) : (
          ...
          <View style={styles.statusRow}>
            <View style={[
              styles.statusDot,
              { backgroundColor: isTraded ? Colors.textLight : isPending ? Colors.warning : Colors.success },
            ]} />
          ...
          {sellerAddress ? (
            <View>
              <View style={styles.addressRow}>
                ...
                <Text style={[styles.addressText, { color: theme.textSecondary }]}>{sellerAddress}</Text>
              </View>
              {geocoding ? (
                ...
                <TouchableOpacity activeOpacity={0.95} onPress={openInMaps} style={styles.mapWrapper}>
                  <MapView
                    provider={PROVIDER_DEFAULT}
                    style={styles.map}
                    initialRegion={{ latitude: mapCoords.lat, longitude: mapCoords.lng, latitudeDelta: 0.02, longitudeDelta: 0.02 }}
                    scrollEnabled={false} zoomEnabled={false} pitchEnabled={false} rotateEnabled={false}
                  >
                    <Marker coordinate={{ latitude: mapCoords.lat, longitude: mapCoords.lng }} title={item.user?.full_name || 'Seller'} description={sellerAddress} />
                  </MapView>
                ...
                <View style={[styles.mapPlaceholder, { backgroundColor: theme.background }]}>
                  ...
                  <Text style={[styles.mapLoadingText, { color: theme.textSecondary }]}>Could not load map for this address</Text>
                </View>
              )}
            </View>
          ) : (
            <View style={[styles.mapPlaceholder, { backgroundColor: theme.background }]}>
              ...
              <Text style={[styles.mapLoadingText, { color: theme.textSecondary }]}>Seller has not set a location</Text>
            </View>
          )}
      ...
      {!isOwnItem && (
        <View style={[styles.bottomBar, { backgroundColor: theme.surface, borderTopColor: theme.border }]}>
          {isUnavailable ? (
            // Item traded/pending — only show message button to view chat history
            <TouchableOpacity
              style={[styles.msgBtn, { borderColor: theme.border, flex: 1 }]}
              onPress={handleMessageSeller}
            >
              <Ionicons name="chatbubble-outline" size={18} color={theme.textSecondary} />
              <Text style={[styles.msgBtnText, { color: theme.textSecondary }]}>View Chat</Text>
            </TouchableOpacity>
          ) : (
            // Item available — show message + offer buttons
            <>
              <TouchableOpacity
          ...
          <ScrollView style={styles.modalScroll}>
            ...
            {myItems.length === 0 ? (
              <View style={styles.noItems}>
                ...
                <Text style={[styles.noItemsText, { color: theme.textSecondary }]}>You have no available items to trade.</Text>
                <TouchableOpacity onPress={() => { setShowOfferModal(false); navigation.navigate('AddItem'); }} style={styles.addItemBtn}>
              ...
              myItems.map((myItem) => (
                <TouchableOpacity
                  key={myItem.id}
                  style={[styles.myItemCard, { backgroundColor: theme.card }, selectedItem?.id === myItem.id && styles.myItemCardSelected]}
            ...
            <Text style={[styles.cashHint, { color: theme.textLight }]}>Add cash to balance the value difference.</Text>
...
const styles = StyleSheet.create({
  ...
  backBtn: { position: 'absolute', top: Platform.OS === 'ios' ? 52 : 20, left: 16, width: 40, height: 40, borderRadius: 20, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 4, elevation: 4 },
  categoryOverlay: { position: 'absolute', bottom: 16, left: 16, paddingHorizontal: 12, paddingVertical: 5, borderRadius: BorderRadius.full },
  categoryOverlayText: { fontSize: FontSize.xs, fontWeight: '700' },
  soldBanner: { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 8 },
  soldBannerText: { color: '#fff', fontWeight: '800', fontSize: FontSize.sm, letterSpacing: 1 },
  contentCard: { borderTopLeftRadius: 24, borderTopRightRadius: 24, marginTop: -20, padding: Spacing.lg },
  ...
  title: { fontSize: FontSize.xxl, fontWeight: '800', marginBottom: Spacing.sm, letterSpacing: -0.3 },
  unavailableNotice: { flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: BorderRadius.md, padding: 12, marginBottom: Spacing.md },
  unavailableText: { fontSize: FontSize.sm, fontWeight: '700' },
  ...
  bottomBar: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', padding: Spacing.md, paddingBottom: Platform.OS === 'ios' ? 32 : Spacing.md, borderTopWidth: 1, gap: 10 },
